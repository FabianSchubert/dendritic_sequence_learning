    ...
    for j in tqdm(range(nSweepScale), leave=False):
        for s in tqdm(range(nSamples), leave=False):
            ...
            for k in tqdm(range(1,nTrain), disable=False, leave=False):
                ...
                y[k] = phi(p[k] + d[k])
                ysquAv[k] = ysquAv[k-1] + eps_av * (-ysquAv[k-1] + y[k-1]**2.)

                #wP[k] = wP[k-1] + eps_w * np.outer(y[k-1]*np.tanh(y[k-1] - .5),
                #                                   xP[k-1] - xPav[k-1])
                wP[k] = wP[k-1] + eps_w * np.outer(y[k-1]*(y[k-1] - ysquAv[k-1])/ysquAv[k-1],
                                                   xP[k-1] - xPav[k-1])
            ...
            for k in range(nTest):
                ...
                yTest[k] = phi(pTest[k] - thD[-1])

            pred = np.argmax(yTest,axis=1)
            perf[i,j,s] = (1.*(pred == labTest)).mean()
    
np.savez("./data/point_model/class_perf_point_ndist_scale_"
